 ...
 * units not actually loaded in the transport.  e.g. a Trieme can only hold 2 units, is stacked with 2 High Men spearmen who can't swim and 2 Lizardmen spearmen who can.
 * Selecting only the Trieme will cause all 5 units to move together, with a movement of 2 sea tiles.  For the High Men the movement speed makes sense... the Lizardmen
 ...
 * 
 * Example 1:
 * Sea tile contains 2 Triemes, 3 High Men, 2 Lizardmen.  We click 1 Trieme to move.  Ends up with transports list=the Trieme, and 2 of the High Men and
 * the 2 Lizardmen get automatically added into the units list even if we didn't select them.
 * 
 * Example 2:
 * Land tile contains a flying Trieme, 2 High Men, 2 Lizardmen.  We click the Trieme to move.  This is still a transported movement, its a minor
 * detail that the Lizardmen can't fit inside and can't actually fly :)  (yes actually tested this in the original!)
 * 
 * Example 3:
 * Land tile contains a flying Trieme, 3 High Men, 2 Lizardmen.  We click the Trieme to move.  This is a regular movement since the High Men don't
 * all fit, and so the Trieme would end up in the units list by itself and move alone.  If we individually clicked the entire stack, they'd all end up in the units
 * list, even the Trieme, and move according to their own movement speeds.
 */
public final class UnitStack
{
	...
	private final List<MemoryUnit> transports = new ArrayList<MemoryUnit> ();
	
	/** Normal units in the stack */
	private final List<MemoryUnit> units = new ArrayList<MemoryUnit> ();
	
	/**
	 * @return Transports holding the other units in the stack, if empty list then all units move individually
	 */
	public final List<MemoryUnit> getTransports ()
	{
		return transports;
	}
