...
public class LbxArchiveReader
{
	 ...
	 * @param fileName Filename of the .lbx file
	 ...
	 */
	protected static InputStream readLbxArchiveHeader (String fileName, int subFileNumber)
		throws IOException
	{
		if (subFileNumber < 0)
		...
		int subFileCount = StreamUtils.readUnsigned2ByteIntFromStream (stream, ByteOrder.LITTLE_ENDIAN, "LBX sub file count");
		if (subFileNumber >= subFileCount)
			throw new ArchiveException ("File " + fileName + " has " + subFileCount +
		...
		if (StreamUtils.readSigned4ByteIntFromStream (stream, ByteOrder.LITTLE_ENDIAN, "LBX signature") != LBX_SIGNATURE)
			throw new ArchiveException ("File " + fileName + " does not have expected LBX signature in bytes 3 thru 6");
	 ...
	 */
	public static InputStream getSubFileInputStream (String fileName, int subFileNumber)
		throws IOException
	{
		InputStream stream = readLbxArchiveHeader (fileName, subFileNumber);
		...
		long bytesToSkip = offset - bytesRead;
	 ...
	 */
	public static ImageInputStream getSubFileImageInputStream (String fileName, int subFileNumber)
		throws IOException
	{
		InputStream stream = readLbxArchiveHeader (fileName, subFileNumber);
		...
		long ending = StreamUtils.readUnsigned4ByteLongFromStream (stream, ByteOrder.LITTLE_ENDIAN, ".lbx sub file ending");
		bytesRead = bytesRead + 4;
