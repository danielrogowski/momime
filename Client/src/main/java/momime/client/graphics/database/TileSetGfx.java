...
public final class TileSetGfx extends TileSet
{
	...
	private Map<String, SmoothingSystemGfx> smoothingSystemsMap;
	...
	final void buildMaps () throws MomException, RecordNotFoundException
	{
		...
		smoothingSystemsMap = new HashMap<String, SmoothingSystemGfx> ();
		
		for (final SmoothingSystem ss : getSmoothingSystem ())
		{
			final SmoothingSystemGfx ssex = (SmoothingSystemGfx) ss;
		...
		{
			final SmoothingSystemGfx ss = smoothingSystemsMap.get (tt.getSmoothingSystemID ());
			if (ss == null)
				throw new RecordNotFoundException (SmoothingSystem.class, tt.getSmoothingSystemID (), "TileSetGfx.buildMaps");
					
			final SmoothedTileTypeGfx ttex = (SmoothedTileTypeGfx) tt;
	 ...
	 */
	public final SmoothingSystemGfx findSmoothingSystem (final String smoothingSystemID, final String caller) throws RecordNotFoundException
	{
		final SmoothingSystemGfx found = smoothingSystemsMap.get (smoothingSystemID);
		if (found == null)
			throw new RecordNotFoundException (SmoothingSystemGfx.class, smoothingSystemID, caller);
	 ...
	 */
	public final SmoothedTileTypeGfx findSmoothedTileType (final String overlandMapTileTypeID, final Integer planeNumber, final String combatTileTypeID)
		throws RecordNotFoundException
	{
		...
		SmoothedTileTypeGfx match = null;
		final Iterator<SmoothedTileType> iter = getSmoothedTileType ().iterator ();
		while ((match == null) && (iter.hasNext ()))
		{
			final SmoothedTileTypeGfx thisTileType = (SmoothedTileTypeGfx) iter.next ();
		...
		if (match == null)
			throw new RecordNotFoundException (SmoothedTileTypeGfx.class, overlandMapTileTypeID + "/" + planeNumber + "/" + combatTileTypeID, "findSmoothedTileType");
